...
trait SensusTrait {
    ...
    {
        return $query->withoutGlobalScope(new SensusScope)->whereNull($this->getTable().'.id_sensus IS NOT NULL');
    }

    public function scopeNotSensus(Builder $query)
    {
        return $query->withoutGlobalScope(new SensusScope)->whereRaw($this->getTable().'.id_sensus IS NULL');
    }
